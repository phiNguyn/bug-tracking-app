...
export default function DashboardPage() {
  ...
  const [chartData, setChartData] = useState({
    bugsByDev: [] as Array<{ developer: { name: string }; count: number; penalty: number }>,
    bugsBySprint: [] as Array<{ sprint: { name: string }; count: number; penalty: number }>,
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        ...
        <Suspense fallback={<Skeleton className="h-10 w-[400px]" />}>
          <DashboardFilters developers={filterOptions.developers} sprints={filterOptions.sprints} />
        </Suspense>
      </div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {new Intl.NumberFormat("vi-VN", { style: "currency", currency: "VND" }).format(stats.totalPenalty)}
            </div>
